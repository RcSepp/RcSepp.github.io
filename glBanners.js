	...
	canvas.onclick = function(event) {
		...
		if (p[0] >= canvasBorderWidth && p[0] < canvasBorderWidth + bannerWidth)
			for (var i = 0; i < banners.length; ++i)
				...
				{
					if (event.ctrlKey)
						window.open("./content.html?view=" + makeId(banners[i].content.title, banners[i].content.id),'_blank');
					else
						window.location.href = "./content.html?view=" + makeId(banners[i].content.title, banners[i].content.id);
					break;
				}
	};
